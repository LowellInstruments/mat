...
def _nok(s):
    return 1, '{} {} error'.format(AG_GPS_ERROR, s)


def _exc(s):
    return 2, '{} {} exception'.format(AG_GPS_EXCEPTION, s)


def _ok(s):
    return 0, '{} {}'.format(AG_GPS_OK, s)


class AgentN2LH_GPS(threading.Thread):
    def __init__(self, q1, q2):
        """ creates an agent for simpler GPS controller """
        super().__init__()
        self.lc = None
        self.q_in = q1
        self.q_out = q2
