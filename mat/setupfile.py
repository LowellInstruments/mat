...
class SetupFile:
    ...
    intervals = np.array([1, 2, 5, 10, 15, 20, 30, 60, 120,
                          300, 600, 900, 1800, 3600])
    interval_string = np.array([
    ...
    def available_tri(self):
        ...
        """
        if self.orient_interval == 0:  # when orient is disabled,
            return self.intervals > 0  # all values are available

        is_available = np.logical_or(
            self.intervals % self._setup_dict['ORI'] == 0,
    ...
    def available_ori(self):
        ...
        is_available = np.logical_or(
            self.intervals % self._setup_dict['TRI'] == 0,
    ...
    def temperature_enabled(self, state):
        ...
        if state is False:  # if temperature logging is disabled
            self._setup_dict['TRI'] = 1  # set recording interval to 1 second
    ...
    def orient_interval(self, value):
        ...
        #     raise ValueError('orientation burst count may not exceed '
        #                      'orientation interval multiplied by '
        #                      'orientation burst rate')
...
def _cfg_header():
    return (
        '// Lowell Instruments LLC - MAT Data Logger - Configuration File\r\n'
