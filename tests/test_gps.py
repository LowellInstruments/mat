...
class FakeSerial:
    def __init__(self, fakeport, fakebaudrate):
        print("mocked fakeserial constructor")
        pass

    def readline(self):
        return b'$GPGGA,150455.000,4138.4356,N,07037.2775,W,2,06,1.6,15.9,M,-34.3,M,3.0,0*1234'
...
class TestGPS:
    ...
    def test_gps_read_line(self):
        with _patch_serial():
            o = GPS("any", 115200)
            assert o.read_line() == b'$GPGGA,150455.000,4138.4356,N,07037.2775,W,2,06,1.6,15.9,M,-34.3,M,3.0,0*1234'

    def test_gps_parse_line_gsa_ok(self):
        with _patch_gps_read_line(b'$GPGGA,150455.000,4138.4356,N,07037.2775,W,2,06,1.6,15.9,M,-34.3,M,3.0,0*1234'):
            o = GPS("any", 115200)
            assert True == o.parse_line()

    def test_gps_parse_line_gmc_ok(self):
        with _patch_gps_read_line(b'$GPRMC,150455.000,A,4138.4356,N,07037.2775,W,0.00,61.63,211118,,,D*1234'):
            o = GPS("any", 115200)
            assert True == o.parse_line()
    ...
    def test_my_measures(self):
        with _patch_serial():
            ...
            o.rmc = rmc_dict
            o.gga = gga_dict
            expected = {'rmc_timestamp': '2018-11-21 18:21:52',
                        'gga_longitude': '69.9835',
